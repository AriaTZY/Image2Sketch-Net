...
def make_gif(sess, pasting_func, data, init_cursor, image_size, infer_lengths, init_width,
    ...
    for round_idx in range(len(infer_lengths)):
        ...
        for round_inner_i in range(round_length):
            ...
            # pencil state, 0 is drop, 1 is lift
            print('info ', round_inner_i, '\n +start point:', cursor_pos*image_size, '\n +stop point:', x2y2, '\n +pen stat:', pen_state)
            import cv2 as cv
            img_show = np.array(gt_stroke_img*255, np.uint8)
            cv.imshow('d', img_show)
            cv.waitKey(0)
            ...
            img_show = np.array(canvas_vis * 255, np.uint8)
            cv.imshow('d', img_show)
            cv.waitKey(0)
...
if __name__ == '__main__':
    ...
    args.file = 'E:/Postgraduate/virtual_sketching-main/outputs/sampling/clean_line_drawings__pretrain_clean_line_drawings/seq_data/0046_0.npz'
